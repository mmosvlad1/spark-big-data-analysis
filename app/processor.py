...
import os
from analytics import run_analytics as analytics_run_analytics, run_business_questions as analytics_run_business_questions
...
def compute_dataset_overview(*args, **kwargs):
    from analytics import compute_dataset_overview as impl
    return impl(*args, **kwargs)


def compute_numeric_stats(*args, **kwargs):
    from analytics import compute_numeric_stats as impl
    return impl(*args, **kwargs)


def compute_segmented_kpis(*args, **kwargs):
    from analytics import compute_segmented_kpis as impl
    return impl(*args, **kwargs)


def run_analytics(spark: SparkSession, df: DataFrame, analytics_root: str) -> None:
    analytics_run_analytics(spark, df, analytics_root)
...
def run_business_questions(spark: SparkSession, df: DataFrame, out_root: str, holidays_df: DataFrame = None, weather_df: DataFrame = None) -> None:
    analytics_run_business_questions(spark, df, out_root, holidays_df=holidays_df, weather_df=weather_df)